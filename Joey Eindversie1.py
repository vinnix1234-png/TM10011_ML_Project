...
hyperparameter_grid_logistic_regression = [
    {
        ...
        'solver': ['saga'],        
        'penalty': ['elasticnet'],
        'l1_ratio': [0.1, 0.5, 0.9]   # 0.0 = L2, 1.0 = L1, tussenin = elasticnet, penalty hoeft niet.
    },
    {
        'C': [0.001, 0.01, 0.1, 1],
        'solver': ['saga'],         
...
# Nested CV
# Cross-validaties
outer_cv = StratifiedKFold(n_splits=5, shuffle=True, random_state=42) # shuffle --> samples worden door elkaar gehusseld
inner_cv = StratifiedKFold(n_splits=5, shuffle=True, random_state=42) # random_state --> zo krijg je steeds dezelfde splits als je de code opnieuw runt
    ...
    for fs_name, fs in feature_selection_methods.items():
        ...
        try:
            ...
            for outer_fold, (outer_train_rijen, outer_test_rijen) in enumerate(outer_cv.split(X, y), start=1):
                ...
                if isinstance(hyperparameter_grids[clf_name], (list, tuple)):
                    hyperparameter_grid = [
                        {f"classifier__{k}": v for k, v in grid.items()}
                        for grid in hyperparameter_grids[clf_name]
                    ]
                else:
                    hyperparameter_grid = {
                        f"classifier__{k}": v for k, v in hyperparameter_grids[clf_name].items()
                     }
